'use client'
import { Zap, Trash2, Edit2, CheckCircle, XCircle, Star, Loader2, AlertTriangle, Clock } from 'lucide-react'
...
// Fallback nếu provider type không có trong PROVIDER_META (vd: openrouter chưa được thêm)
const FALLBACK_META = { label: 'Unknown', color: '#64748b', models: [] }

const QUOTA_STATUS: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ok:        { label: 'Bình thường',  color: 'text-green-600',  icon: CheckCircle },
  limited:   { label: 'Gần hết',      color: 'text-yellow-600', icon: AlertTriangle },
  exhausted: { label: 'Hết quota',    color: 'text-red-600',    icon: XCircle },
  error:     { label: 'Lỗi',          color: 'text-red-600',    icon: XCircle },
  unknown:   { label: 'Chưa kiểm tra', color: 'text-gray-400',  icon: Clock },
}

export function ProviderCard({ provider, testing, onEdit, onDelete, onTest, onSetDefault, onToggleActive }: Props) {
  const meta = PROVIDER_META[provider.provider] ?? FALLBACK_META
  const quotaInfo = QUOTA_STATUS[provider.quota_status ?? 'unknown']
  const QuotaIcon = quotaInfo.icon

  return (
    <div className={`relative rounded-xl border bg-white p-5 shadow-sm transition-all hover:shadow-md ${
      provider.is_default ? 'border-indigo-300 ring-1 ring-indigo-200' : 'border-gray-200'
    }`}>
      {/* Default badge */}
      {provider.is_default && (
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex items-start gap-3 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-0.5">{meta.label} · <span className="font-mono">{provider.model}</span></p>

            {/* Quota status */}
            <div className={`mt-1 flex items-center gap-1 text-[11px] font-medium ${quotaInfo.color}`}>
              <QuotaIcon size={11} />
              {quotaInfo.label}
              {provider.last_error && provider.quota_status === 'error' && (
                <span className="ml-1 text-[10px] text-gray-400 font-normal truncate max-w-[160px]" title={provider.last_error}>
                  — {provider.last_error.slice(0, 40)}
                </span>
              )}
            </div>
        ...
        <div className="flex shrink-0 items-center gap-1">
          ...
          <Tooltip content={provider.is_default ? 'Đang là mặc định' : 'Đặt làm mặc định'} side="top">
            <button onClick={onSetDefault}
              className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${
                provider.is_default ? 'text-amber-500' : 'text-gray-400 hover:bg-gray-100 hover:text-amber-500'
              }`}>
          ...
          <Tooltip content={provider.is_active ? 'Tắt' : 'Bật'} side="top">
            <button onClick={onToggleActive}
              className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${
                provider.is_active ? 'text-emerald-500 hover:bg-emerald-50' : 'text-gray-400 hover:bg-gray-100'
              }`}>
      ...
      <div className="mt-4 flex gap-4 border-t pt-3 text-xs text-gray-500">
        ...
        {provider.last_tested_at && (
          <span className="ml-auto">Test lúc: <strong className="text-gray-700">
            {new Date(provider.last_tested_at).toLocaleString('vi-VN', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })}
          </strong></span>
        )}
      </div>
    </div>
