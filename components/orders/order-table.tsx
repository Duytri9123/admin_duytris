...
export function OrderTable() {
  ...
  const { data, isLoading, isError } = useQuery({
    ...
    queryFn: () =>
      api.get<PaginatedResponse<Order>>('/api/admin/orders', { page, per_page: 10, status: statusFilter || undefined }).then((r) => r.data),
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3">
        ...
        <select value={statusFilter} onChange={(e) => { setStatusFilter(e.target.value as OrderStatus | ''); setPage(1) }} className="rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
              ...
              {orders.map((order) => (
                <tr key={order.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-medium ${STATUS_COLORS[order.status]}`}>{STATUS_LABELS[order.status]}</span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/dashboard/orders/${order.id}`} className="rounded px-2 py-1 text-xs font-medium text-blue-600 hover:bg-blue-50">Chi tiết</Link>
                  </td>
                </tr>
              ))}
            </tbody>
